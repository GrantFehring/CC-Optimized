This is a personal project meant to help optimally solve the "Commander Codex" online game found at https://commandercodex.com/
...
If the best_guess function has outputted a small data frame with card characteristics rather than an avg column, that means 2 or less answers remain, so there's no calculations to be done
best_guess can be used to find the best first guess if it is run with no arguments
  It will take much, much longer than when provided with a guess because all guesses remove a significant number of solutions
