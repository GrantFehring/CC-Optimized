...
CCjson = jsonlite::fromJSON("https://storage.googleapis.com/legend-tutor.appspot.com/daily.json")
```
...
for (i in 1:n) {
  ...
  # nice and easy
  cmc = api_df$cmc
  # extract only month and year of date, since that is what CC uses
  date = substr(api_df$released_at, start = 1, stop = 7)
...
best_guess = function(
    ...
    guess5 = NULL, result5 = NULL, guess6 = NULL, result6 = NULL) {
  # reset solutions df
  solutions_df <<- df
  
  # vectorize guesses and results
  guesses <- c(guess1, guess2, guess3, guess4, guess5, guess6)
  results <- c(result1, result2, result3, result4, result5, result6)
  # Filter possible answers based on previous guesses using vectorized operation
  for (i in seq_along(guesses)) {
    if (!is.null(guesses[i]) && !is.null(results[i])) {
